...
function execSync(command,callback){
	...
	try {
		...
		console.debug(err.stdout + " " + err.sterr);
	}
	return childProcess;
}
...
function kill_dnsmasq_and_hostapd(callback){
	exec("pkill hostapd",function handle_error(err,data){
		if(err){
...
function wpa_config(ssid,passphrase,options,callback){
	...
	exec('rm -rf ' + file,function(err,data){
		...
		fs.writeFile(file, 
			...
			'}\n'
		,function(err,data){
			exec('wpa_supplicant -B -i ' + options.INTERFACE + ' -c ' + file,function(err,data){
				if(err){
					console.error(err);
					callback(err,null);
				}
...
function removeWificonnectorOnBoot(callback){
	exec("sudo systemctl disable wifi-connector",function(err,data){
		if(err){
			console.error(err);
			calback(err,null);
		}else{
			console.info("wifi-connector service stopped");
			callback(null,'done')
		}
	});
}

function checkInternet(options,cb){
...
function addGoogleNameServers(options,callback){
	checkInternet(options,function(isConnected){
		if(!isConnected){
			exec("sudo rm -rf /etc/resolv.conf",function(err,data){
				if(err){
					callback("error during removing resolv.conf",null);
				}else{
					exec("cat > /etc/resolv.conf << EOF	\nnameserver 8.8.8.8\nnameserver 8.8.4.4\n",function(err,data){
						if(err){
							callback("error during writing google nameservers in resolv.conf",null);			
						}else{
							console.info("google nameservers set in resolv.conf");
							callback(null,'done')
						}
					});;
				}	
			...
			console.info("still connected to internet");
			callback(null,'done')
		}
	});
}
...
exports.addGoogleNameServers = addGoogleNameServers;
