...
function checkInternet(options,callback){
	var execit = require('child_process').exec, child;

	child = execit('ping www.google.be', function(error, stdout, stderr){
		if(error !== null){
			console.debug("Google Not available");
			callback(error,null);
		}else{
			console.debug("Google Available");
			callback(null,'done');
		}
	});
}

function addGoogleNameServers(options){
	checkInternet(options,function(err,data){
		if(err){
			execSync("sudo rm -rf /etc/resolv.conf");
			execSync("cat > /etc/resolv.conf << EOF	\nnameserver 8.8.8.8\nnameserver 8.8.4.4\n");
