...
var options = {
	...
	DRIVER: 'nl80211',
	AP_SSID: 'Pi3-AP',
	TMP_DIR: '/tmp/.wifi-connector-tmp',
	LOG_DIR: '/var/log'
}
...
function init(callback) {
	...
	fs.exists(options.TMP_DIR,function(exists){
	    ...
	    execSync('lsb_release -a | tee .wifi-connector-tmp/lsb_release.txt');
	    var output = fs.readFileSync('.wifi-connector-tmp/lsb_release.txt');
		version = output.toString().match(/Distributor ID:\t*([^\n\r]*)/)[1];
			//select appropriate version
			switch(version){
				case "Raspbian":
					raspbian.init(options);
					callback(null,"Found Raspbian");
					break;
				default:
					console.error("Environment not detected");
					callback(new Error("Environment not detected"),null);
					break;
			}
	});
}
