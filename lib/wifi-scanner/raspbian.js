...
function init(options){
	...
	config.exec("sudo systemctl disable hostapd",function handle_error(err,data){
		config.exec("sudo systemctl stop hostapd",function handle_error(err,data){
			console.debug("Stopped Hostap service");
		});
	});
	config.exec("sudo systemctl disable dnsmasq",function handle_error(err,data){
		config.exec("sudo systemctl stop dnsmasq",function handle_error(err,data){
			console.debug("Stopped Dnsmasq");
		});
	});
}
	...
	if(err){
		...
		if(err){
			...
			if(err){
				...
				config.exec("sudo pkill wpa_supplicant",function handle_error(err,data){
				config.exec("sudo pkill wpa",function handle_error(err,data){
