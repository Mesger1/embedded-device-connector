...
function removeHardcodedWPA(callback){
	...
	fs.readFile(source_file_path, 'utf8', function (rfErr, rfData) {
		...
		if(fileData.indexOf('#   wpa-conf /etc/wpa_supplicant/wpa_supplicant.conf') > -1){
			console.warn("global wpa_supplicant file already disabled");
			callback(null,"global wpa_supplicant file already disabled");
			return;
		}else{
