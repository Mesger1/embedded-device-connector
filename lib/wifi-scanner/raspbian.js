...
function isRunning(name,callback){
	exec("ps -cax | grep " + name,function(err,data,stderr){
		if(err){
			console.system(name + " not running");
			callback(err,0);
		}else{
			if(data){
				console.system(name + " running");
				callback(null,1);
			}else{
				console.system(name + " not running");
				callback(null,0);
...
function setupAP(callback){
	...
	removeHardcodedWPA(function(err,data){
		...
		console.system("hardcoded wpa removed");
		isRunning("wpa",function(err,data){
			if(data == 1 ){
				exec("sudo pkill wpa",function(err,data){
					...
					console.system("wpa_supplicant killed");
				});
			}
			exec("sudo systemctl daemon-reload",function(err,data){
				...
				console.system("daemon reloaded");
				...
				exec("sudo service dhcpcd restart",function(err,data){
					...
					console.system("dhcpcd restarted");
					...
					down(options.INTERFACE,'0.0.0.0',function(err,data){						
						...
						console.system("ifconfig down 0.0.0.0");
						...
						up(options.INTERFACE,'0.0.0.0',function(err,data){
							...
							console.system("ifconfig up 0.0.0.0");
							...
							kill_dnsmasq_and_hostapd(function(err,data){
								...
								console.system("dnsmasq and hostapd killed");
								...
								down(options.INTERFACE,options.AP_IP_ADDRESS,function(err,data){
									...
									console.system("ifconfig down " + options.INTERFACE);
									...
									up(options.INTERFACE,options.AP_IP_ADDRESS,function(err,data){
										...
										console.system("ifconfig up " + options.INTERFACE);
										...
										dnsmasq_enable(dnsmasqoptions,function(err,data){
											...
											console.system("dnsmasq options set");
											...
											console.system("hostapd enabled");				
...
function connectToWifi(ssid,passphrase,callback){
	...
	kill_dnsmasq_and_hostapd(function(err,data){
		...
		console.system("hostapd and dnsmasq killed");
					...
					down(options.INTERFACE,'0.0.0.0',function(err,data){						
						...
						up(options.INTERFACE,'0.0.0.0',function(err,data){
							...
							wpa_enable(wpa_options,function(err,data){
								...
								console.system("wpa up on interface : " + wpa_options.interface);
								console.system("wpa up on ssid : " + wpa_options.ssid);
								console.system("wpa up with driver : " + wpa_options.driver);
