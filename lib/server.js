...
wifiscanner.start = function(){
   app.use(express.static('../public'))
    ...
    app.get('/',function (req, res) {
      var path_str = __dirname.replace('/lib','');
      res.sendFile(path_str  + "/public/index.html");
      res.end();
    ...
    app.post('/connect', function(req, res) {
            ...
            var command = exec("sudo " + dirname__ + "/../setup_wifi_connection.sh " + essid + " " + passphrase, function (error, stdout, stderr) {
                if(error){
                    console.log(stderr);
                }
            });
            command.on('close', function(code) {
                ...
                if(code==0){
                    ...
                    exec("sudo " + dirname__ + "/../setup_wifi_apn.sh", function (error, stdout, stderr) {
                        if(error){
                            console.log(stderr);
                        }
                    });
